...
**Hold 长条的梯形**（实现说明）：长条沿下落方向**跨越一段距离**，两端在倾斜面上的深度差看得见 —— 远端必须更窄，所以整条长条应当是个梯形。Canvas2D 的一次仿射变换画不出梯形（只能得到平行四边形），因此长条在 `|sinθ| > 0` 时改走**分段投影**（`src/render/canvas2d.js` 的 `drawTiltedHoldBands`）：

- **先按源像素切片**（头尾帽 / 主体 / 光效，见 `hold-geometry.js` 与 `textures.js`），再把每一片按**设备像素**分段：段高 4 设备像素（`HOLD_TILT_BAND_PX`，单段上限 400 段）；
- **段的上下边界取设备 y 的整数倍**，段内的局部 y 由投影**反解**得到（`makeDeviceYSolver`：三点定标拟合成分式线性 + 割线法收敛 + 二分兜底；投影仍是唯一出处，渲染器不另抄一份公式）；
- 每段画**一次**：一个**精确四边形裁剪**（四个角都是投影出来的位置 —— 倾斜面上直线投影后仍是直线，实测偏离 ~1e-13px，所以四角四边形就是这段的真实形状）+ 一次三点定标的仿射铺贴。整幅横向铺满，**不拆三角形、也不按横向结构分块**：没有对角线就没有折角，无旋转时 `k` 只随局部 y 变化、段内近似常数，横向映射在段内是线性的，本体的左右边缘因此天然落在正确位置（HL 贴图同样处理，不需要额外分块）；
- **为什么必须对齐设备网格**（这是接缝的根因）**【实测】**：一次「clip + drawImage」的边界像素，其抗锯齿覆盖**不是**相邻两块互补的 —— 覆盖之和只取决于这条共享边离设备像素网格多远：水平共享边落在 `y = 200` 时边界像素是 255（严丝合缝），落在 `y = 200.3` 时只有 207，落在 `y = 200.5` 时只有 191（少 25%）；斜边同理。于是每条不在网格上的共享边都会留下一条暗线。**把裁剪路径外扩来补缝无效**：补上「缝」就会换成同样明显的「亮带」（半透明光效被画两遍）。对齐到设备整数后覆盖是 0/1 判定，相邻段共享同一条精确边界 —— 真浏览器实测：长条中段逐行亮度最深凹陷从 **-21.5%** 降到 **-0.7%**；
- **切片接缝也吸附**到最近的设备整数 y（吸附量 ≤ 半个设备像素，内容只平移不到 1px）：贴上之后相邻切片共用同一条边界，`hold-geometry.js` 里那 1px 的「重叠补缝」在分段画法下不再需要（重叠对半透明贴图会叠出亮带）；
- 深度被投影的 `MIN_DEPTH` 夹住时，局部 y → 设备 y 会**折返**（背面 + 大倾斜实测能把 8 个局部像素拉到 1150 设备像素，画出来是一个大漏斗）。分段画法先把绘制区间裁到**屏幕窗口**（上下各留 8px 余量），并把**映射单调**的那一段以外的部分丢掉（`monotoneLocalRange`）；屏幕外的片直接跳过；
- **屏幕外的段整段剔除**（长条常常一多半在画面外）；`renderer.stats` 记录 计划 / 实际绘制 / 剔除 的**段**数；
- 宽度从贴线一端向远端连续收窄（梯形），头尾帽各自按自己那一端的 `k` 缩放；判定线旋转时远端沿线方向横向偏移；
- 倾斜为 0（或「垂直判定」的 `ignore3D`）时走原来的单次变换路径，与旧版本逐像素一致；
- **判定线被旋转**（`worldRotate ≠ 0`）时，段的边界在屏幕上不再水平、无法对齐设备网格，退回旧的逐行两三角形画法（`drawTiltedHoldRows`，18px/24 行 + `HOLD_TILT_SEAM_BLEED_PX` 外扩）：这种配置下仍会留下淡淡斜缝（实测最深 **-4%**，明显好于本轮的 -21%，但没有根治）；没有 `clip` 的环境同样走这条退路。
...
- **本项目的游玩结算**：玩家模式下要等**音频完全播完**（`audioSettled()`：音频位置到达 `duration`，容差 0.05 s）才显示结算页 —— 音乐比谱面长时先放完尾奏，音乐先结束（谱面更长）则直接结算。本项目不打官方那 0.22049 s 的提前量（用户要求「音频完全播放完成后」）；
...
| Hold 绘制 | ✅ | 长度与尾部位置按 `note.holdSpeed` 口径算（§4.2：`'own'` 独立 / `'line'` 跟随判定线，后者的头尾都乘 `speed`、命中后尾部仍跟线，**贴线瞬间长度不跳变**）。固定分段：**主体 = 本体（core）去掉两端卡口**（`capTop ~ core.h - capBottom`，主体结束处就是帽开始处、不漏源行），帽与光效都是**固定 48 源像素**高度；HL 贴图本体外那一圈光效按「非 HL 贴图 + 本体外各 48px 光效」处理；切片间重叠 1px 消除接缝（分段画法下不需要，接缝吸附到设备整数）；尾部 / 头部随时间收拢；**下落面倾斜时按设备像素分段投影成梯形**（`drawTiltedHoldBands`：段高 4 设备像素、段边界取设备整数 y、每段一次精确四边形裁剪 + 一次仿射、整幅横向铺满；切片接缝吸附到设备整数；屏幕外的段跳过），倾斜为 0 时走原来的单次变换路径（逐像素与旧版一致）；判定线被旋转时退回旧的逐行两三角形画法（`drawTiltedHoldRows`） |
...
- 位置全部经渲染器投影：`judgeBand()` 给出带中心与半宽，`toLineChart()` 把屏幕点**解析地**换算回判定线局部坐标，`hitJudgeBand()` 判点、`hitJudgeBandSegment()` 判「滑动是否经过带」；
- 判据只有一条：点击 / 滑动端点在**判定线局部坐标**里的横向坐标与音符所在列相差不超过半宽。沿下落方向的那一维不参与判断，因此判定线上下两侧无限远都算；
- 半宽 = **音符宽 × 0.8**（两边各 80% 音符宽，判定宽度 = 音符宽的 160%；`JUDGE.BAND_HALF_RATIO`）；音符宽与「列位置」都在 z = 0 空间里算（像素）；
- 判定线旋转时带子跟着转；`positionX` 偏移的音符带子跟着走；Hold 用**头部**所在的列；
- **两档的差别只在「换算时是否考虑深度」**（`opts.ignore3D`）：
  - 垂直判定（`ignore3D = true`）：忽略相机 / z / 倾斜 —— 屏幕点直接读成 2D 的局部坐标，判定范围是**等宽长条**；
  - 轨道判定（默认）：用解析逆（`localX = u/k − a₀`、`localY0 = (v·D₀ − F·b₀)/(v·sinθ + F·cosθ)`，`(u,v)` 是把屏幕偏移转进判定线方向后的分量）把手指换算回**倾斜面**上，于是判定范围是**跟着下落面收敛的楔形**：越远越窄、并随判定线方向偏移，同时仍然「沿下落方向不限位置」；
  - 两种模式的判定区域都能由 `judgeBandShape()` 画成屏幕多边形；调试叠加层「显示判定范围」画的就是它，并跟随暂停页里选的那一档（全屏判定时不画）；
- ⚠️ 背面音符的判定范围与同 `positionX` 的正面音符是**同一列**：背面音符只是从另一侧落下、贴图旋转 180°，不会把列镜像到另一边（`noteTransform()` 里 `localX` 取反、角度 +π 是**绘制**约定；`toLineChart()` 用同一套约定换算，两边自洽）。
