...
| 真实游玩（触屏） | ✅ | 判定范围默认「垂直判定」（见 §7.3，判定带两边各 80% 音符宽），可切全屏判定；Tap ±0.08/±0.18/±0.22 s，Hold 无 Bad；多指判定（每根手指独立记账 + `touches` 对账，界面上的手指不拖累别的手指）；Hold **允许换手**，持续时间内判定范围里有任一根手指即算保持、断连 ≤80 ms 接上不算断；Drag 需判定时刻有手指在带里；Flick 简单判定：窗口内任一手指在判定范围里（按着 / 划过）即 Perfect |
