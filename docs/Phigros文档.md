...
| Hold | 头部贴线、尾巴收回来，直到尾部过线 | 半透明继续下落，头尾一起走，直到尾部也过线才消失（不淡出） |
...
| 真实游玩（触屏） | ✅ | 判定范围默认「垂直判定」（见 §7.3），可切全屏判定；Tap ±0.08/±0.18/±0.22 s，Hold 无 Bad；多指判定；Hold 需按住到尾部；Drag 需判定时刻有手指在带里；Flick 滑动经过带即 Perfect |
| 计分 / 连击 / ACC / 物量 | ✅ | 按 §5.2 的公式；分数显示用 `0 + 定点` 格式 |
| 打击音效 | ✅ | `assets` 下三种音效；Hold 重复特效不重复播放；加载或播放失败自动静默关闭。⬜ RPE 逐音符 `hitsound` 未播放 |
| 评级（φ / V / S / A…） | ⬜ | |
| 开场 / 结束动画 | ⬜ | 按项目范围不做 |
