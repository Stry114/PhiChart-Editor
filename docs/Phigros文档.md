...
- 谱面相机写在**根节点**的自有扩展键 `camera` 里：`{ xEvents, yEvents, zEvents, focalEvents }`，与扩展事件同构（拍值 + 起止值 + 缓动）。单位：`x` 用长度单位（`1350` = 一个画面宽）、`y` / `z` / `focal` 用长度单位（`900` = 一个画面高；`focal` 缺省 `900` = 默认焦距）。详见 §3.5.1。
...
- `F` = 焦距（画面高，缺省 `PSEUDO3D.FOCAL_H = 1`）；
...
- `z = 0` 且相机在缺省位置（`Cx = Cy = Cz = 0`、`F = 1`）时 `k = 1`，画面与「没有 3D」逐像素一致（所有旧公式都不变）；
- 判定线的 `z > 0` → 整条线（含其上的音符、线长与线厚）缩小并向画面中心靠拢；
- 相机的 `z` 通道（`Cz > 0`，往屏幕内推）→ `k > 1` → 画面整体放大、透视更强；相机 `x` / `y` 平移 → 画面整体反向平移，且**近处移动多、远处移动少**（视差）；
- 焦距 `F` 只改透视强弱：z = 0 平面上的东西大小恒为 1:1，越远的点对 `F` 越敏感（`F` 越大越接近正交）。
...
| `focal` | 焦距（越小透视越强） | 画面高比例（缺省 1） | `focalEvents`（长度单位，900 = 缺省焦距） |

- 求值：每帧从 `chart.cameraRt` 求出 `state.camera = { x, y, z, focal }`，投影层按它变换（见上面的公式）；
- 缺省（无关键帧）= `{ x: 0, y: 0, z: 0, focal: 1 }`，画面与完全没有相机时一致；
...
| 谱面相机 `camera`（根节点）**【本项目】** | ✅ | 谱面级关键帧 `x` / `y` / `z` / `focal`（四个通道各有列表，支持缓动 / 贝塞尔）；每帧求值后供投影使用。见 §3.5.1 与 §7.3 |
...
| （伪）3D 投影 | ✅ | 小孔相机（§3.5）：`k = F/(F + z − Cz)`，位置与尺寸一起乘 k；`z` / `theta` 事件 + 谱面相机 `x`/`y`/`z`/`focal` 共用同一套公式；缺省参数下与 2D 逐像素一致；打击特效按命中时刻的相机快照定位与缩放 |
| 音符绘制（Tap / Drag / Flick） | ✅ | 普通 / `*HL` 两套贴图；背面音符旋转 180° |
| Hold 绘制 | ✅ | 固定分段（48px 帽 + 48px 光效，按本体裁切）固定高度 + 主体拉伸；切片间重叠 1px 消除接缝；尾部 / 头部随时间收拢 |
