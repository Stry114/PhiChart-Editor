...
**Hold 长条的梯形**（实现说明）：长条沿下落方向**跨越一段距离**，两端在倾斜面上的深度差看得见 —— 远端必须更窄，所以整条长条应当是个梯形。Canvas2D 的一次仿射变换画不出梯形（只能得到平行四边形），因此长条在 `|sinθ| > 0` 时改走**逐行投影**（`src/render/canvas2d.js` 的 `drawTiltedHold`）：

- **先按源像素切片**（头尾帽 / 主体 / 光效，见 `hold-geometry.js` 与 `textures.js`），再把每段沿下落方向**逐行**投影：18px 一行、上限 24 行（行数按屏幕长度自适应）；
- 每行还要按贴图的**横向结构**拆成若干块：HL 贴图（双押 / 多押）在本体（`meta.core`）左右各有一圈 48px 光效，行内「竖直」的源列（本体的左右边缘）在两个三角形的仿射下会在共用对角线处折一个角 —— 若本体边缘落在块内部，折角就落在长条**实体的边缘**上，表现为边缘不整齐的台阶（普通贴图本体就是整行，所以只在开双押换成 HL 贴图时才暴露）。拆到本体 / 光效的边界上之后，折角只留在贴图内部（颜色与透明度连续处），看不见；
- 每块的**四个角全部精确投影**：倾斜面上的一条直线投影后仍是**直线**（实测偏离 ~1e-13px），所以「本地矩形[列 × 行]」的真实形状就是四角投影出来的四边形；
- 每块拆成**两个裁剪三角形**（左上/右上/左下 与 右上/右下/左下），每个三角形用自己那三点定标的仿射铺贴图 —— 于是贴图的左右两条边严格落在真实投影边界上。只用一个仿射时只能对上三个角，第四条边必然偏出去（实测 18px 行高就有 3~5px），而长条贴图的左右边缘是**可见**的，每条边错开一点就是肉眼可见的锯齿；
- 分段解决的是**透视**：整段只用一个仿射的话，贴图在长度方向会被「拉直」，远端看起来不对（分段越细越准、越费；18px/24 行是实测的平衡点）；
- **拼接缝**：一次「clip + drawImage」的边界像素会被抗锯齿算两遍（clip 的覆盖 × 贴图自身的边缘覆盖），两块紧挨着画会留下约 1/3 像素的缝（背景透出来就是一条暗线，实测最暗处比周围暗 ~53/159）。做法是把**绘制矩形与源矩形**（不含裁剪路径）往相邻块方向外扩 `HOLD_TILT_SEAM_BLEED_PX = 0.75` 个屏幕像素（纵向向下一行、横向向相邻的块），让贴图自身的边缘落到裁剪路径之外 —— 覆盖只由 clip 决定，相邻两块共享同一条**精确**边界、覆盖互补，既没有缝也不会重叠；
  ⚠️ **裁剪路径不能一起外扩**：两块重叠绘制同一像素时 `source-over` 会把半透明的部分叠加两次，HL 贴图本体外那圈光效会亮一档，表现为长条上一道道横向亮带（拼接处的透明度叠加）；
- **屏幕外的行整行剔除**（长条常常一多半在画面外）；`renderer.stats` 记录 计划 / 实际绘制 / 剔除 的**行**数（横向分块不单独计数）；
- 宽度从贴线一端向远端连续收窄（梯形），头尾帽各自按自己那一端的 `k` 缩放；判定线旋转时远端沿线方向横向偏移；
- 倾斜为 0（或「垂直判定」的 `ignore3D`）时走原来的单次变换路径，与旧版本逐像素一致；
- 没有 `clip` 的环境（测试桩件 / 很老的浏览器）退回单仿射的 `drawImage`：不会缺块，第四条边允许小偏差（此路径不外扩绘制矩形，相邻块严丝合缝）。
