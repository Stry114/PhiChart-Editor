// 倾斜 Hold 的渲染取舍：把「为什么要逐行分段 / 为什么一行要用两个三角形」量化出来。
//
// 结论（也是 src/render/canvas2d.js 里 drawTiltedHold 的实现依据）：
//   1. 一行的真实形状 = 四个角精确投影出来的**四边形**：倾斜面上的一条直线投影后仍是直线
//      （下面 `boundaryLineDeviation()` 实测偏离 0.000px），所以只要四个角对，轮廓就精确。
//   2. 三点定标的仿射（drawImage 能做的）只能对上三个角，第四条边必然偏出去：
//      `affineFourthCornerError()` 给出偏差随行高的**线性**增长 —— 18px 行高就有 3~5px、
//      θ=70° 时 40px 以上，正好是肉眼可见的锯齿台阶。故每行改用「两个三角形 + 裁剪」，
//      每个三角形的三个角都精确（贴图左右两条边因此严格落在真实边界上）。
//   3. 分段仍然必要：整段贴图用一次仿射会被「拉直」（长度方向的透视丢失，远端明显不对），
//      而只靠加密行数把误差压到 0.4px 需要 300~1000 行 —— 所以取「18px 一行、上限 24 行」
//      这个平衡点：`rowsNeededForError()` 就是这条结论的数据。
//
